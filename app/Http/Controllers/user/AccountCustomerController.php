...
//        }
    ...
    public function actived(AccountCustomer $customer,$token)
    {
        if($customer->token ===$token)
        {
            ...
            if(Auth::guard('account_customer')->attempt($login)) {
                return redirect()->route('shopping.login')->with('status','Xác minh thành công!');;
            }else{
        ...
        if($accountcustomer===null){
            ...
            try{
                ...
                if (!$mail->Send()) {
                    ...
                    return redirect()->back()->with('forgot','Chúng tôi không thể gửi email xác minh cho bạn được.');
//                echo "<h1>Loi khi goi mail: " . $mail->ErrorInfo . '</h1>';
                } else {
                    return redirect()->back()->with('forgot','Làm ơn click vào đường link chúng tôi đã gửi qua email của bạn để đổi lại mật khẩu!');
                }
            }catch (Exception $e) {
                return redirect()->back()->with('forgot','Đã có lỗi xảy ra! Lấy lại mật khẩu không thành công');
            }
        }
...
//            $customer->update(['forgot'=>null]);
    ...
    public function check_change_pass(Request $request)
    {
        ...
        if($pass===$comfirm_pass){
            ...
            if($accountcustomer===null){
                return redirect()->back()->with('change','Mật khẩu không chính xác. vui lòng nhập lại!');
            }else{
                $accountcustomer->update(['forgot'=>null,'password'=>Hash::make($pass)]);
                if(Auth::guard('account_customer')->check()){
                    Auth::guard('account_customer')->logout();
                    return redirect()->route('shopping.login')->with('status','Đổi mật khẩu thành công. Hãy đăng nhập lại!');
                }else{
                    return redirect()->route('shopping.login')->with('status','Đổi mật khẩu thành công!');
                }
            }
        }else{
            return redirect()->back()->with('change','Nhập lại mật khẩu không trùng khớp!');
        }
